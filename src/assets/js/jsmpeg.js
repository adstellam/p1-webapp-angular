...
JSMpeg.Player = function() {
	...
	Player.prototype.showHide = function(ev) {
		if (document.visibilityState==="hidden") {
			this.unpauseOnShow=this.wantsToPlay;this.pause()
		} else 
			if (this.unpauseOnShow) {
				this.play()
			}
	};
	...
	Player.prototype.updateForStaticFile = function() {
		...
		if (this.audio&&this.audio.canPlay) {
			...
			if (this.video) {
				...
				headroom = this.demuxer.currentTime-targetTime}this.source.resume(headroom);
				if (notEnoughData&&this.source.completed) {
					...
					if (notEnoughData&&this.options.onStalled) {
...
JSMpeg.BitBuffer = function() {
	...
	var BitBuffer = function(bufferOrLength,mode) {
		...
		this.mode = mode||BitBuffer.MODE.EXPAND;this.index=0
	};
	...
	BitBuffer.prototype.evict = function(sizeNeeded) {
		...
		this.byteLength = this.byteLength-bytePos;this.index-=bytePos<<3;
		return
	};
	...
	BitBuffer.prototype.findNextStartCode = function() {
		for (var i=this.index+7>>3;i<this.byteLength;i++) {
			if (this.bytes[i]==0&&this.bytes[i+1]==0&&this.bytes[i+2]==1) {
				this.index = i+4<<3;return this.bytes[i+3]
			}
		}
		this.index = this.byteLength<<3;return-1
	};
	...
	BitBuffer.prototype.peek = function(count) {
		...
		while (count) {
			...
			value=value<<read|(currentByte&mask<<shift)>>shift;offset+=read;
			count-=read
		}
		return value
	};
	...
	BitBuffer.prototype.has = function(count) {
		return(this.byteLength<<3)-this.index>=coun
	};
...
JSMpeg.Source.Fetch = function() {
	...
	FetchSource.prototype.pump = function(reader) { return reader.read().then(function(result) { 
		if (result.done) { 
			this.completed=true
		} else { 
...
JSMpeg.Source.AjaxProgressive=function() {
	...
	var AjaxProgressiveSource=function(url,options) {
		this.url=url;this.destination=null;
		this.request=null;
		this.streaming=false;
		this.completed=false;
		this.established=false;
		...
		this.isLoading=false;this.loadStartTime=0;
		this.throttled=options.throttled!==false;this.aborted=false;
		this.onEstablishedCallback=options.onSourceEstablished;
		this.onCompletedCallback=options.onSourceCompleted;
	};
...
JSMpeg.Demuxer.TS=function() {
	...
	TS.prototype.parsePacket=function() {
		...
		var daptationField=this.bits.read(2);
	...
	TS.prototype.packetStart=function(pi,pts,payloadLength){
		pi.totalLength=payloadLength;pi.currentLength=0;pi.pts=pts
	};
	...
	TS.prototype.packetComplete=function(pi){
		pi.destination.write(pi.pts,pi.buffers);pi.totalLength=0;pi.currentLength=0;pi.buffers=[]
	};
	TS.STREAM={
		PACK_HEADER:186,SYSTEM_HEADER:187,PROGRAM_MAP:188,PRIVATE_1:189,PADDING:190,PRIVATE_2:191,AUDIO_1:192,VIDEO_1:224,DIRECTORY:255
	};
	return TS
}();
...
JSMpeg.Decoder.MPEG1Video=function() {
	...
	MPEG1.prototype.decodePicture=function(skipOutput) {
		this.currentFrame++;this.bits.skip(10);
		...
		while(code>=MPEG1.START.SLICE_FIRST&&code<=MPEG1.START.SLICE_LAST){
			this.decodeSlice(code&255);code=this.bits.findNextStartCode()
		}
		if(code!==-1){
			this.bits.rewind(32)
		}
	...
	MPEG1.prototype.copyMacroblock=function(motionH,motionV,sY,sCr,sCb) {
		...
		width=this.halfWidth;scan=width-8;
		H=motionH/2>>1;
		V=motionV/2>>1;
		oddH=(motionH/2&1)===1;
		oddV=(motionV/2&1)===1;
		...
		if(oddH){
			if(oddV){
				...
				while(dest<last){
					...
					src++;
					for(x=0;x<2;x++){
						cr2=sCr[src];cb2=sCb[src++];cr=cr1+cr2+1>>1&255;cb=cb1+cb2+1>>1&255;cr1=sCr[src];cb1=sCb[src++];cr|=cr1+cr2+1<<7&65280;
						cb|=cb1+cb2+1<<7&65280;cr2=sCr[src];cb2=sCb[src++];cr|=cr1+cr2+1<<15&16711680;cb|=cb1+cb2+1<<15&16711680;cr1=sCr[src];
						cb1=sCb[src++];cr|=cr1+cr2+1<<23&4278190080;cb|=cb1+cb2+1<<23&4278190080;dCr[dest]=cr;dCb[dest]=cb;dest++
					}
					dest+=scan>>2;
					src+=scan-1
				}
			...
			if(oddV) {
				while(dest<last){
					for(x=0;x<2;x++){
						...
						src++;cr|=sCr[src]+sCr[src+width]+1<<7&65280;
						cb|=sCb[src]+sCb[src+width]+1<<7&65280;
						src++;cr|=sCr[src]+sCr[src+width]+1<<15&16711680;cb|=sCb[src]+sCb[src+width]+1<<15&16711680;
						src++;cr|=sCr[src]+sCr[src+width]+1<<23&4278190080;cb|=sCb[src]+sCb[src+width]+1<<23&4278190080;
						src++;dCr[dest]=cr;dCb[dest]=cb;dest++
					}
					dest+=scan>>2;
					src+=scan
				}
				...
				while(dest<last){
					for(x=0;x<2;x++){
						cr=sCr[src];cb=sCb[src];
						src++;cr|=sCr[src]<<8;
						cb|=sCb[src]<<8;src++;cr|=sCr[src]<<16;cb|=sCb[src]<<16;src++;cr|=sCr[src]<<24;cb|=sCb[src]<<24;
						src++;dCr[dest]=cr;dCb[dest]=cb;dest++
					}
					dest+=scan>>2;
					src+=scan
	...
	MPEG1.prototype.decodeBlock=function(block){
		...
		if(this.macroblockIntra){
			...
			quantMatrix=this.intraQuantMatrix;n=1
		}else{
		...
		if(block<4){
			...
			destArray=block===4?this.currentCb:this.currentCr;scan=(this.codedWidth>>1)-8;
	...
	MPEG1.CopyValueToDestination=function(value,dest,index,scan){
		for(var n=0;n<64;n+=8,index+=scan+8){
			dest[index+0]=value;dest[index+1]=value;dest[index+2]=value;
			dest[index+3]=value;dest[index+4]=value;dest[index+5]=value;
	...
	MPEG1.IDCT=function(block) {
		...
		for(var i=0;i<8;++i){
			...
			b7=tmp1+tmp2;m0=block[0*8+i];x4=(b6*473-b4*196+128>>8)-b7;
			x0=x4-((tmp1-tmp2)*362+128>>8);x1=m0-b1;x2=((block[2*8+i]-block[6*8+i])*362+128>>8)-b3;x3=m0+b1;
			y3=x1+x2;y4=x3+b3;y5=x1-x2;y6=x3-b3;y7=-x0-(b4*473+b6*196+128>>8);
			block[0*8+i]=b7+y4;block[1*8+i]=x4+y3;block[2*8+i]=y5-x0;block[3*8+i]=y6-y7;block[4*8+i]=y6+y7;block[5*8+i]=x0+y5;block[6*8+i]=y3-x4;
			block[7*8+i]=y4-b7
		}
		for(var i=0;i<64;i+=8){
			b1=block[4+i];b3=block[2+i]+block[6+i];b4=block[5+i]-block[3+i];tmp1=block[1+i]+block[7+i];
			tmp2=block[3+i]+block[5+i];b6=block[1+i]-block[7+i];b7=tmp1+tmp2;m0=block[0+i];x4=(b6*473-b4*196+128>>8)-b7;
			x0=x4-((tmp1-tmp2)*362+128>>8);x1=m0-b1;x2=((block[2+i]-block[6+i])*362+128>>8)-b3;x3=m0+b1;
			y3=x1+x2;y4=x3+b3;y5=x1-x2;y6=x3-b3;y7=-x0-(b4*473+b6*196+128>>8);block[0+i]=b7+y4+128>>8;block[1+i]=x4+y3+128>>8;
			block[2+i]=y5-x0+128>>8;block[3+i]=y6-y7+128>>8;block[4+i]=y6+y7+128>>8;block[5+i]=x0+y5+128>>8;block[6+i]=y3-x4+128>>8;
	...
	MPEG1.PICTURE_TYPE={
		INTRA:1,PREDICTIVE:2,B:3
	};
	MPEG1.START={
		SEQUENCE:179,SLICE_FIRST:1,SLICE_LAST:175,PICTURE:0,EXTENSION:181,USER_DATA:178
	};
	return MPEG1
}();
...
JSMpeg.Decoder.MP2Audio=function(){
	...
	MP2.prototype.decodeFrame=function(left,right){
		...
		if(version===MP2.VERSION.MPEG_2){sampleRateIndex+=4;bitrateIndex+=14}
				...
				for(var p=0;p<3;p++){
					...
					for(var ch=0;ch<2;ch++){
						...
						while(vIndex<1024){
							...
							vIndex+=128-32;dIndex+=64-32
						}
						vIndex=128-32+1024-vIndex;dIndex-=512-32;
						while(vIndex<1024){
							for(var i=0;i<32;++i){
								this.U[i]+=this.D[dIndex++]*this.V[vIndex++]
							}
	...
	MP2.prototype.readSamples=function(ch,sb,part){
		...
		if(sf===63){sf=0}else{
			var shift=sf/3|0;sf=MP2.SCALEFACTOR_BASE[sf%3]+(1<<shift>>1)>>shift
		}
		var adj=q.levels;
		if(q.group){
			val=this.bits.read(q.bits);sample[0]=val%adj;
			val=val/adj|0;sample[1]=val%adj;
			sample[2]=val/adj|0
		}else{
		...
		val=(adj-sample[1])*scale;sample[1]=val*(sf>>12)+(val*(sf&4095)+2048>>12)>>12;
		val=(adj-sample[2])*scale;sample[2]=val*(sf>>12)+(val*(sf&4095)+2048>>12)>>12
	};
...
JSMpeg.Renderer.WebGL=function() {
	...
	WebGLRenderer.prototype.allowsClampedTextureData=function() {
		...
		var texture=gl.createTexture();
		/*
		var pixel = new Uint8ClampedArray([0]);
		*/
		var pixel = new Uint8Array([0]);
		gl.bindTexture(gl.TEXTURE_2D,texture);
	...
	WebGLRenderer.prototype.renderProgress=function(progress) {
		...
		gl.uniform1f(loc,progress);gl.drawArrays(gl.TRIANGLE_STRIP,0,4)
	};
	...
	WebGLRenderer.prototype.updateTexture=function(unit,texture,w,h,data) {
		var gl=this.gl;gl.activeTexture(unit);
		gl.bindTexture(gl.TEXTURE_2D,texture);
...
JSMpeg.Renderer.Canvas2D=function(){
	...
	CanvasRenderer.prototype.destroy=function(){this.canvas.remove()};
	...
	CanvasRenderer.prototype.YCbCrToRGBA=function(y,cb,cr,rgba){
		...
		for(var row=0;row<rows;row++){
			for(var col=0;col<cols;col++){
				...
				rgba[rgbaIndex1+2]=y1+b;rgba[rgbaIndex1+4]=y2+r;
				rgba[rgbaIndex1+5]=y2-g;rgba[rgbaIndex1+6]=y2+b;
				rgbaIndex1+=8;
				var y3=y[yIndex2++];
				var y4=y[yIndex2++];
				rgba[rgbaIndex2]=y3+r;rgba[rgbaIndex2+1]=y3-g;
				rgba[rgbaIndex2+2]=y3+b;rgba[rgbaIndex2+4]=y4+r;
				rgba[rgbaIndex2+5]=y4-g;rgba[rgbaIndex2+6]=y4+b;
				rgbaIndex2+=8
			}
			yIndex1+=yNext2Lines;
			yIndex2+=yNext2Lines;
			rgbaIndex1+=rgbaNext2Lines;
			rgbaIndex2+=rgbaNext2Lines;cIndex+=cNextLine
		}
	};
...
JSMpeg.WASMModule=function(){
	...
	var WASM=function(){
		...
		WASM.prototype.c_sbrk=function(size){
			var previousBrk=this.brk;this.brk+=size;
