...
@Component({
	selector: 'app-vedas-user-add',
	templateUrl: './vedas-user-add.component.html',
	styleUrls: ['./vedas-user-add.component.css']
})
export class VedasUserAddComponent implements OnInit {
	...
	addVedasUser() {
		if (this.isVedasUserIdUnique()) {
	    	...
	    	this.http.post<VedasUser>(`${environment.apiUrl}/vedasUsers`, this.vedasUserAdded, { headers })
	      		.subscribe(
	      			vedasUser => { console.log("VedasUser added.") },
	      			err => { console.log(err) }
	      		);
	      	if (this.vedasUserAddForm.value.role == 'driver') {
	      		this.addDriverAndDriverStatus();
	      		window.alert("To complete this task, please go to the 'Data Management > Manage Driver > Edit' page to fill in required data fields.");
	      	}
	      	this.activeModal.close();
	      	setTimeout(() => {
	            this.router.navigateByUrl('/', { skipLocationChange: true })
		            .then(() => {
		                this.router.navigate(['/customer-home', this.cid, 'vedas-user-management']);
		            });
        	}, 1000);
      	}
	}
	...
	assignVedasUserAddFormData() {		
		...
		this.vedasUserAdded.role = this.vedasUserAddForm.value.role;
